...
namespace Item.Inventory
{
    public class Inventory : MonoBehaviour, ISaveable, IFreezeMovement
    {
        ...
        public class InventoryEventDispatcher
        {
            ...
            public InventoryEventDispatcher(GameObject gameObject, Inventory inventory)
            {
                ...
                for (int i = 0; i < loadInterfaces.Count; i++)
                {
                    ...
                    // Also dispatch invisible items (e.g. gold/currency)
                    foreach (InventoryItem item in inventory.invisibleItems)
                    {
                        loadInterfaces[i].OnItemLoaded(-1, item.Data, item.Amount);
                    }
                }
            }
        ...
        public void UseItem(int slotIndex, bool ignoreFreeze = false)
        {
            if (isMovementFrozen && !ignoreFreeze)
                return;
        ...
        public void OnLoad(string data)
        {
            ...
            if (inventorySaveData.savedItems.Length != 0)
            {
                ...
                for (int i = 0; i < inventorySaveData.savedItems.Length; i++)
                {
                    ...
                    {
                        Debug.Log($"[Inventory] OnLoad: FAILED to find ItemData for guid: {getSave.guidString}");
                    }
                }
            ...
            {
                Debug.Log("[Inventory] OnLoad: savedItems is empty, NOT restoring obtainedStartingItems");
            }
        }
