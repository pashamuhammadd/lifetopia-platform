...
namespace Item.Inventory
{
    public class InventorySlot : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler, IRecieveItemSlotIcon
    {
        ...
        public class Settings
        {
            ...
            public bool useItemOnRightClick = true;
        }
        ...
        {
            // Right click or special 'use' condition
            if (eventData.button == PointerEventData.InputButton.Right || 
                (eventData.button == PointerEventData.InputButton.Left && settings.useItemOnRightClick && eventData.clickCount > 1))
            {
                if (references.Inventory != null)
                {
                    references.Inventory.UseItem(slotIndex, true);
                }
                return;
            }

            // Sell mode: clicking sells the item
            if (ShopManager.Instance != null && ShopManager.Instance.IsSellMode)
            {
                ShopManager.Instance.Sell(slotIndex);
                return;
            }
        ...
        {
            if (ItemTooltip.Instance != null && currentItemData != null)
            {
                ItemTooltip.Instance.Show(currentItemData, eventData.position);
            }
        }
