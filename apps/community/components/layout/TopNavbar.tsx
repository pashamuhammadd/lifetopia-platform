...
export async function TopNavbar({ profile }: { profile: CurrentProfile | null }) {
  ...
  const [unreadNotifications, unreadMessages] = profile
  ...
  return (
    <header className="sticky top-3 z-30 mb-4 rounded-[24px] border border-[#ead9b8] bg-white/88 p-3 shadow-[0_18px_45px_rgba(88,60,28,0.10)] backdrop-blur-xl md:top-4 md:p-4">
      <div className="flex items-center gap-3">
        ...
        <Link
          href="/explore"
          className="hidden min-h-11 flex-1 items-center gap-3 rounded-[18px] border border-[#ead9b8] bg-[#fffaf0] px-4 text-sm font-bold text-[#9b6635] transition hover:border-[#6fa83a] md:flex"
        >
          <Compass size={18} />
          <span>Search posts and Lifetopians</span>
        </Link>
        <div className="ml-auto flex items-center gap-2">
          <AuthenticatedLink
            ...
            className="relative grid size-11 place-items-center rounded-full bg-[#fff7e8] text-[#7a5635]"
          >
            <Bell size={19} />
            {unreadNotifications ? (
              <span className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-[#c12626] px-1 text-[10px] font-black text-white">
          ...
          <AuthenticatedLink
            href="/messages"
            requiresAuth
            aria-label={`Open messages${unreadMessages ? `, ${unreadMessages} unread` : ""}`}
            className="relative grid size-11 place-items-center rounded-full bg-[#fff7e8] text-[#7a5635]"
          >
