...
export async function searchExplore(query: string, tab: "posts" | "people", page: number) {
  ...
  if (tab === "people") {
    const [{ data, error }, { data: authData }] = await Promise.all([
      supabase.rpc("search_community_profiles", { p_query: query, p_limit: 13, p_offset: offset }),
      supabase.auth.getUser(),
    ]);
    const rows: ExploreProfileRow[] = error || !data ? [] : (data as ExploreProfileRow[]);
    return {
      profiles: rows
        .slice(0, 12)
        .map((item) => ({
          ...
          avatarSrc: `/images/avatars/${item.avatar_id}.jpg`,
          role: formatCommunityRole(item.role),
          followersCount: Number(item.followers_count ?? 0),
          viewerIsFollowing: Boolean(item.viewer_is_following),
          isOwnProfile: authData.user?.id === item.id,
  ...
  const rows: ExplorePostRow[] = error || !data ? [] : (data as ExplorePostRow[]);
  return {
    profiles: [],
    posts: rows
      .slice(0, 12)
      .map((item) => ({
        ...
        createdAt: new Date(item.created_at).toLocaleDateString("en", {
          month: "short",
          day: "numeric",
        }),
