-- Corrected upgrade for an existing Community Phase 4 installation.
-- The earlier failed transaction was rolled back; run this complete file once.
begin;
...
drop function if exists public.get_my_community_quests();

create function public.get_my_community_quests()
...
create function public.claim_daily_community_quest(p_quest_code text)
...
revoke all on function public.get_my_community_quests(),public.claim_daily_community_quest(text) from public,anon;
grant execute on function public.get_my_community_quests(),public.claim_daily_community_quest(text) to authenticated;
commit;
