...
create table if not exists public.community_quest_claims (
  id bigint generated always as identity primary key,
  user_id uuid not null references public.profiles(id) on delete cascade,
  quest_code text not null check (quest_code in ('daily_login', 'read_posts', 'like_posts', 'meaningful_comment', 'create_post', 'daily_community')),
...
returns table (
  ...
  completed boolean, reward integer, claimed boolean, period_ends_at timestamptz,
...
language sql stable security definer set search_path = pg_catalog, public as $$
  ...
  from quests q cross join account a cross join day d;
$$;
...
begin
  ...
  if exists(select 1 from public.community_quest_claims where user_id=v_user and quest_code in (p_quest_code,'daily_community') and period_key=v_day) then
  ...
  values(v_user,4,v_balance,'daily_quest',p_quest_code||':'||v_day::text,'Claimed community quest: '||replace(p_quest_code,'_',' '));
...
revoke all on function public.record_community_quest_event(text,uuid), public.get_my_community_quests(), public.claim_daily_community_quest(text) from public, anon;
grant execute on function public.record_community_quest_event(text,uuid), public.get_my_community_quests(), public.claim_daily_community_quest(text) to authenticated;
